...
"""
import os, datetime, torch, transformers
...
log("START elicit_ground_truth_v2.py")
...
n_flag = 0
for iid, wdesc, tdesc, _ in ITEMS:
    w, t = PAIRS[iid]
...
rep_txt = "\n".join(rep)
print(rep_txt)
with open(os.path.join(OUTDIR, "tokenization.txt"), "w", encoding="utf-8") as f:
    f.write(rep_txt + "\n")
log(f"Part 1 done: tokenization.txt written; {n_flag}/{len(ITEMS)} pairs flagged for token-count mismatch (reported only, nothing dropped).")
...
log(f"{len(prompts)} prompts built (parallel17 bare skipped: shares 'the parallel' with parallel38).")
...
tsv = os.path.join(OUTDIR, "ground_truth_v2.tsv")
